...
def _read_gold_and_pred(gold_fpath, pred_fpath):
    ...
    with open(gold_fpath, encoding='utf-8') as gold_f:
        ...
        for line_res in gold_f:
            (_, tweet_id, check_worthiness) = line_res.strip().split('\t')  # process the line from the res file
    ...
    with open(pred_fpath) as pred_f:
        for line in pred_f:
            _, _, tweet_id, score, _  = line.split('\t')
...
def get_mean_values(gold_files, pred_files):
    ...
    if pred_files_count > 1:
        overall_precisions = [item * 1.0 / pred_files_count for item in overall_precisions]
